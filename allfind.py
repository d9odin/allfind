...
class ThreadCallstack:
    ...
    def get_module_imports(
        cls, context, module_pe, mod_layer_name, module_name, module, import_func
    ):

        module_base = module.DllBase
        module_size = module.SizeOfImage
        try:
            image_import_dir_addr = (
                module_base
                + module_pe.get_nt_header().OptionalHeader.DataDirectory[1].VirtualAddress
            )
            image_import_dir_size = module_pe.get_nt_header().OptionalHeader.DataDirectory[1].Size
        except:
            return

        layer = context.layers[mod_layer_name]

        image_import_dir_data = layer.read(image_import_dir_addr, image_import_dir_size, pad=True)

        if len(image_import_dir_data) > 0:
            d = int(image_import_dir_size / 20)
            for i in range(d):
                dll_name = layer.read(
                    module_base
                    + struct.unpack("<l", image_import_dir_data[(i * 20) + 12 : (i * 20) + 16])[0],
                    120,
                    pad=True,
                )
                try:
                    import_func.append((dll_name.split(b"\x00")[0].decode()).lower())
                except:
                    pass
    ...
    @classmethod
    def get_module_exports(cls, context, module_pe, mod_layer_name, module_name, module):
        export_func = {}

        module_base = module.DllBase
        module_size = module.SizeOfImage
        try:
            image_export_dir_addr = (
                module_base
                + module_pe.get_nt_header().OptionalHeader.DataDirectory[0].VirtualAddress
            )
            image_export_dir_size = module_pe.get_nt_header().OptionalHeader.DataDirectory[0].Size
        except:
            return

        layer = context.layers[mod_layer_name]

        image_export_dir_data = layer.read(image_export_dir_addr, image_export_dir_size, pad=True)

        if len(image_export_dir_data) > 0:
            num_of_funcs = struct.unpack("<l", image_export_dir_data[0x14:0x18])[0]
            num_of_names = struct.unpack("<l", image_export_dir_data[0x18:0x1C])[0]
            addr_of_names = module_base + struct.unpack("<l", image_export_dir_data[0x20:0x24])[0]
            addr_of_ordinals = (
                module_base + struct.unpack("<l", image_export_dir_data[0x24:0x28])[0]
            )
            addr_of_funcs = module_base + struct.unpack("<l", image_export_dir_data[0x1C:0x20])[0]

            for i in range(num_of_names):
                fname_rva = struct.unpack("<l", layer.read(addr_of_names + (i * 4), 4, pad=True))[0]
                if (fname_rva in (0, None)) or (fname_rva > module_size):
                    continue
                ordinal = struct.unpack("<H", layer.read(addr_of_ordinals + (i * 2), 2, pad=True))[
                    0
                ]
                if (ordinal == None) or (ordinal >= num_of_funcs):
                    continue
                func_rva = struct.unpack(
                    "<l", layer.read(addr_of_funcs + (ordinal * 4), 4, pad=True)
                )[0]
                if (func_rva in (0, None)) or (func_rva > module_size):
                    continue

                n = layer.read(module_base + fname_rva, 128, pad=True).split(b"\x00")[0].decode()
                f = module_base + func_rva

                if (module_base + func_rva >= image_export_dir_addr) and (
                    module_base + func_rva < image_export_dir_addr + image_export_dir_size
                ):
                    d = "{0} Forwads to {1}".format(ordinal, n)
